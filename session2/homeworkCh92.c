...
int getFForDate(int year, int month){
  if(month <= 2){
    printf("year is %i and month is %i and year-1 is %i \n", year, month, year-1);
    return year - 1;
  } else {
    printf("year is %i and month is %i \n", year, month);
    return year;
  }
}

int getGForDate(int month){
  if(month <= 2){
    printf("month is %i and month + 13 is %i \n", month, month + 13);
    return month + 13;
  } else {
    printf("month is %i and month + 1 is %i \n", month, month + 1);
    return month + 1;
  }
}

int getNForDate(struct date dayToN){
  return 1461 * getFForDate(dayToN.year, dayToN.month) / 4 + 153 * getGForDate(dayToN.month) / 5 + dayToN.day;
}
...
int main(void){
  ...
  secondDate.month = 8;
  secondDate.day = 8;
  secondDate.year = 2004;

  int nForFirstDay = getNForDate(firstDate);
  printf("nForFirstDay is %i\n", nForFirstDay);
  int nForSecondDay = getNForDate(secondDate);
  printf("nForSecondDay is %i\n", nForSecondDay);
  int difference = nForFirstDay - nForSecondDay;
